...
def researcher(state: AgentState):
    # 실제 구현에서는 이 함수가 리서치 작업을 수행.
    # 여기서는 임의로 관련 데이터를 찾는 척 한다.
    response = {
        'role': 'assistant',
    ...
    # 임의의 데이터 생성
    fake_data = {
        'data': '전세계 인구 데이터: [미국: 331M, 중국: 1.4B, 인도: 1.3B, 대한민국:5.0M]'
    }
    response['content'] += f'\n찾은 데이터: {fake_data['data']}'
    return {'messages': [response]}

def coder(state: AgentState):
    # 실제 구현에서는 이 함수가 코드를 작성.
    # 여기서는 임의로 코드를 작성하는 척 한다.
    response = {
        'role': 'assistant',
...
graph.get_graph().draw_mermaid_png(output_file_path='graph_supervisor.png')
# 예시
initial_state = {
    'messages': [
        {
